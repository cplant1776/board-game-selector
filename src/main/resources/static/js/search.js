$("#submissionBtn").click(function()
    {
        ...
        var checkedCategories = $("input:checked")
        var categories = '';
        ...
        // Sort param
        sortParam = '&sort=gameRank';
        // Get form parameters
        var values = $("form").serialize();
        // Redirect to url
        // window.location.href = "/query?" + values + catParam + sortParam;
        window.location.href = "/query?" + values + sortParam;
    }
);
...
$(document).ready(function() {
    ...
    for (var i = 0; i < categories.length; i++) {
        ...
        addedCategoriesRow += '<div class="form-check form-check-inline"><input class="form-check-input" type="checkbox" id="'+categories[i]+'" value="'+categories[i]+'"><label class="form-check-label" for="'+categories[i]+'">'+categories[i]+'</label></div>';
    }
    $('.modal-body').html(addedCategoriesRow);
    */
 });